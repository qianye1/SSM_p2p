...
import com.gxa.cdut.util.MailFiled;
...
public class MailVerifyServiceImpl implements IMailService {
    ...
    @Autowired
    private IDetailService iUserInfoService;
    ...
    public void sendVerifyEmail(String email) {
        //判断用户是否已经绑定邮件
        Detail userInfo = iUserInfoService.selectById(UserContext.getLoginInfo().getId());
        if (!userInfo.getIsBindEmail()) { //如果当前用户没有绑邮箱
            ...
            String content =
                    "<html>" +
                            ...
                            "<h3><a href='http://localhost:8081/bindEmail.do?code=" + uuid + "'>" +
            ...
            try {
                ...
                mv.setLogininfo_id(userInfo.getId());
                mv.setEmail(email);
                mv.setUuid(uuid);
    ...
    //发送邮件方法
    private void sendMail(String email, String content, Detail userInfo, String uuid)
